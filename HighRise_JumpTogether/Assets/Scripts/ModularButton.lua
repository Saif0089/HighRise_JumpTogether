...
local reset_response = Event.new("ResetResponse")
...
function self:ServerAwake()
    ...
    collision_exit_request:Connect(function(player) --[[[Disable on server here]] collision_exit_response:FireAllClients()  print("Server Received Exit Request") end)
    Timer.Every(5, function() EnableObject(false) end)
end
